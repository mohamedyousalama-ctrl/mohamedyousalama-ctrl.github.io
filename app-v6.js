...
const EXAM_MS = 55 * 60 * 1000;
const STORE_KEY = "tw_academy_v6";
...
function lockKey(email){return "tw_lock_v6_"+(email||"").toLowerCase()}
...
let tickTimer=null;
let cloudCache=[];
function materialize(q){return q}
function activeQuestions(){return QUESTIONS.map(materialize)}
function startClock(){
  stopClock();
...
function payload(){
  ...
  return {version:6,brand:"THE WEEKEND",profile:state.profile,answers:state.answers,startedAt:state.startedAt,elapsedMs:usedMs(),elapsed:fmt(usedMs()),submittedAt:state.submittedAt,score:sc,review:qs.map(function(q){return {id:q.id,title:q.title,type:q.type,answer:state.answers[q.id]==null?null:state.answers[q.id],correct:q.correct||null,ok:q.correct?String(state.answers[q.id])===String(q.correct):null}})} ;
}
